#!/usr/bin/env python3
"""
Automated gradient-size sweep for ring_simulation_v2.py

Runs the fat-tree ring all-reduce simulation for each gradient size
in sequence, handling the Containernet CLI interaction and cleanup
between runs.

Usage (from /home/mahmoud/networks/containernet):
    python3 run_ring_gradient_sweep.py
"""
...
ORIG_SCRIPT = os.path.join(BASE_DIR, "examples", "ring_simulation_v1.py")
TEMP_SCRIPT = os.path.join(BASE_DIR, "examples", "_ring_sweep_temp.py")
...
def run_one(label, total_bytes, venv_path):
    ...
    print(f"\n{'='*70}")
    print(f"  RING GRADIENT SWEEP — {label}  "
    ...
    cmd = [
        ...
        "python3", f"examples/_ring_sweep_temp.py",
    ]
    ...
    # Wait for the process to finish (collect_ring_metrics sleeps 20s internally)
...
def main():
    ...
    print("\n" + "=" * 70)
    print("  SWEEP COMPLETE — check <size>_ring_metrics.log, <size>_ring_link_load.log, and <size>_ring_routes.log")
    print("=" * 70)
